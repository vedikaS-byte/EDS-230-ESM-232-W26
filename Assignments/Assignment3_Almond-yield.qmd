...
Authors: Melannie Moreno, Vedika Shirtekar
...
## Extra the tmax_c, tmin_c, precip columns as variables
tmax_c <- climate %>% filter(month == 2) %>% select(tmax_c)
tmin_c <- climate %>% filter(month == 2) %>% select(tmin_c)
precip <- climate %>% filter(month == 1) %>% select(precip)

## Construct the function
almond_yeild <- function(tmax_c, tmin_c, parameters){……}
