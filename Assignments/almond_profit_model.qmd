...
author: "Melannie Moreno Rolón & Vedika Shirtekar"
format: html
editor_options: 
...
```{r}
# establish baseline 
base_price <- 2.14    # per lb
base_cost  <- 3807   # per acre
...
price_pound <- runif(
  nsamples,
  min = base_price - base_price * price_dev, # do baseline 
  max = base_price + base_price * price_dev
)

cost_acre <- runif(
  nsamples,
  min = base_cost - base_cost * cost_dev, # do baseline 
  max = base_cost + base_cost * cost_dev
)
...
sens_results %>% 
  mutate(price_group = case_when(
    cost_acre < 3615 ~ "Low",
    cost_acre >= 3615 & cost_acre < 3990 ~ "Medium",
    cost_acre >= 3990 ~ "High"
  )) %>% 
  ggplot(
       aes(
         x = factor(price_group,levels = c("Low", "Medium", "High"), ordered = T), 
        y = mean_profit, 
        fill = factor(price_group, levels = c("Low", "Medium", "High"), ordered = T))) +
  geom_boxplot() + 
  scale_fill_manual(
    values = c(
      "Low" = "darkseagreen", 
  ...
  labs(title = "Sensitivity of Almond Profit to Production Cost",
       x = "Cost Category ($/Acre)",
       y = "Mean Annual Profit ($/acre)", 
       fill = "Cost Level Category") +
  theme_bw()
...
sens_results %>% 
  mutate(price_group = case_when(
    price_pound < 2 ~ "Low",
    price_pound >= 2 & cost_acre < 2.3 ~ "Medium",
    price_pound >= 2.3 ~ "High"
  )) %>% 
  ggplot(
       aes(
         x = factor(price_group,levels = c("Low", "Medium", "High"), ordered = T), 
