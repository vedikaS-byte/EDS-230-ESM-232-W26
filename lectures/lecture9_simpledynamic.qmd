...
-   Could run for different locations... different climate inputs (almond yields distributed over space)
...
-   Ex. Sick vs recovered... depends on how many people currently affected.. depends on how many are infected in period before
...
-   **Diffusion** (dynamic in space and time... affects in space and affect at some point in time)
    -   ex. dispersal of air pollutants
-   **Population** (population growth depends on \# of individuals at a particular point in time)
    -   Ex. Population of a virus or microbes during a pandemic
    -   Growth rate depends on the reproductive material available
...
-   **flows** - transfers outcome(s) between variables or from the system
...
-   **State-space**: description of the entity may require multiple variables - for a watershed this could be soil moisture, water currently in dam and water stored in trees, and for each "grid" in a watershed)
    -   All difference states (ex. all "different" watersheds at certain periods of time)
-   **State-space trajectories**: how the system state evolves through time... [**this is what we are modeling**]{.underline}
-   **Initial conditions**: values to describe the system state at the beginning (baseline)
-   High dimensionality
...
-   dynamic systems often have *feedback* loops
    -   positive (something about use generates more use)
    -   negative (reduction in use leads to return to homeostasis in system)
...
-   what variables would define the state-space
    ...
    -   Precipitation, evaporation rate, incoming flows into the bay, drought (labelled by index)

-   what would control the evolution of the state-space

    -   Constrained to specific salinity range based on geography (ex. hypothetically 5-32 psu)
...
Differential equation (foundation of dynamic models, model CHANGE through TIME): - $dP$ / $dt$ = $rP$
