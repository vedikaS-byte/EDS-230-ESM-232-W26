...
-   Sampling (just have to know distribution, not have to have actual data, can simulate!)
...
# Establish distribution of ethresh samples
ethresh <- rnorm(mean = 10000, sd = .15*10000, n = 20)

# Establish distribution of ethresh samples
#eff <- rnorm(mean = 0.6, sd = 0.1, n = 20)
#eff = .8
# Create a list of separate tibbles for the mean electricity generate each year
site3 <- ethresh %>% map(~ solarpv(area = 0.1, 
  solar = sierraczosolar, clr = "green", 
  eunit = "kWhr", g = FALSE, etype = "direct", ethresh = .x))

# Convert site3 list output to data frame  
tmp <- map_dfr(site3, `[`, c("annual"))

tmp_dist <- data.frame(year = tmp$annual$year, elect = tmp$annual$elect )

tmp_dist %>% ggplot(aes(x = year, y = elect, group = year)) + geom_boxplot()
